...
import { HeisenbergDM } from '../@types/HeisenbergDM';
...
export const HDMProvider: React.FC<Props> = ({ children }) => {
  ...
  useEffect(() => {
    async function loadUsuario() {
      const userData = await AsyncStorage.getItem(DMBDSTORE_USER);
      const user: IUser = userData ? JSON.parse(userData) : undefined;

      const storedToken = await AsyncStorage.getItem(DMBDSTORE_TOKEN);
